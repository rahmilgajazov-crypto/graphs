...
// Проверка, что граф корректен (имеет метод vertexCount)
...
function findEulerianPath(graph) {
    ...
    while (edges.length > 0) {
        ...
        for (const edge of candidates) {
            ...
            if (!isBridge(graph, edge, currentId) || candidates.length === 1) {
                nextEdge = edge;
        ...
        if (path.length === 0) path.push(vertices[currentId].label);
