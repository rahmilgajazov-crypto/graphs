 ...
 * Отрисовка на canvas: вершины, рёбра, стрелки (для ориентированных), веса
 * 
 * Соответствует пункту 9 документации
 */

/**
 * Отрисовка стрелки (треугольника) в указанной точке и угле
 * @param {CanvasRenderingContext2D} ctx 
 * @param {number} x 
 * @param {number} y 
 * @param {number} angle - в радианах
 */
function drawArrow(ctx, x, y, angle) {
    ...
    // Цвет стрелки в зависимости от темы
    const isDark = document.body.classList.contains('dark-theme');
    ctx.fillStyle = isDark ? '#fbbf24' : '#2563eb';  // золотой в тёмной, синий в светлой
    ctx.fill();
...
/**
 * Отрисовка прямого ребра со стрелкой (для ориентированных графов)
 * @param {CanvasRenderingContext2D} ctx 
 * @param {Object} from - вершина {x, y}
 * @param {Object} to - вершина {x, y}
 * @param {number|null} weight - вес ребра (если есть)
 */
function drawStraightEdge(ctx, from, to, weight) {
    ...
    // Вес (со смещением от центра, чтобы не перекрываться с другими рёбрами)
    if (weight !== undefined && weight !== null) {
        const isDark = document.body.classList.contains('dark-theme');
        ctx.fillStyle = isDark ? '#f1f5f9' : '#000000';
        ctx.font = 'bold 32px monospace';

        // Перпендикулярное смещение от центра ребра
        const dx = to.x - from.x;
        const dy = to.y - from.y;
        const len = Math.hypot(dx, dy);
        const perpX = -dy * 15 / len;
        const perpY = dx * 15 / len;
...
/**
 * Отрисовка изогнутого ребра (для обратных рёбер при двойных)
 * @param {CanvasRenderingContext2D} ctx 
 * @param {Object} from 
 * @param {Object} to 
...
function drawCurvedEdge(ctx, from, to, weight) {
    ...
    const offset = 45;
    const perpX = -dy * offset / len;
    const perpY = dx * offset / len;
    ...
    // Стрелка на конце кривой (по касательной)
    const t = 0.92;
    const mt = 1 - t;
    ...
    if (weight !== undefined && weight !== null) {
        ...
        ctx.fillText(weight, midX + 15, midY - 15);
    }
}

/**
 * Отрисовка неориентированного ребра (без стрелок)
 * @param {CanvasRenderingContext2D} ctx 
 * @param {Object} from 
 * @param {Object} to 
 * @param {number|null} weight 
